  ...
  const loadData = async (): Promise<void> => {
    ...
    try {
      ...
      if (results[2].status === 'fulfilled') {
        const chaptersData = results[2].value.data.data || results[2].value.data;
        setChapters((chaptersData as any).chapters || chaptersData || []);
      } else {
        console.error('Failed to load chapters:', results[2].reason);
        setChapters([]);
      }

      // Progress - optional
      if (results[3].status === 'fulfilled') {
        setStudyProgress(results[3].value.data.data || results[3].value.data);
      } else {
        console.error('Failed to load progress:', results[3].reason);
      }
